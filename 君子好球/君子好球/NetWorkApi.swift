...
//        password = username.stringByTrimmingCharactersInSet(whitespace)
   ...
   //删除发布的约球信息
    ...
    //点赞
    func zanMessage(user_id:String ,message_id:String ,block : @escaping (_ dict:Dictionary<String, Any>) -> Void){
        let Url = URL.init(string:"http://127.0.0.1:8000/admin/zanMessage")
        let request = NSMutableURLRequest.init(url: Url!)
        request.timeoutInterval = 5
        request.httpMethod = "POST"
        let dict = ["user_id":user_id,"message_id":message_id]
   ...
   //取消点赞
    func canleZanMessage(user_id:String ,message_id:String ,block : @escaping (_ dict:Dictionary<String, Any>) -> Void){
        let Url = URL.init(string:"http://127.0.0.1:8000/admin/canleZanMessage")
        let request = NSMutableURLRequest.init(url: Url!)
        request.timeoutInterval = 5
        request.httpMethod = "POST"
        let dict = ["user_id":user_id,"message_id":message_id]
